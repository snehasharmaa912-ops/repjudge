...
| Category | Max | How RepJudge Covers It |
|:---|:---:|:---|
| **Technical Implementation** | 25 | `st.session_state` for history persistence, `st.form`-free single-call batching per photo, clean Pandas pipeline for score trends, zero-error deploy on Streamlit Cloud |
| **AI Integration & Prompt Engineering** | 20 | Gemini **Vision** multimodality (not just chat), fixed `system_instruction` persona, dynamic f-string checkpoint injection per exercise, enforced structured output contract |
| **UI/UX & Data Visualization** | 20 | Column layout, `st.expander`, `st.metric` with qualitative deltas, `st.line_chart` progress trend, `st.data_editor` session log |
| **Deployment & Cloud Engineering** | 15 | Live on Streamlit Community Cloud, pinned `requirements.txt`, zero local/system dependencies |
| **Open-Source Branding** | 10 | This README — terminal identity, architecture diagram, setup + deploy instructions, live link |
| **System Design & Documentation** | 10 | [`SYSTEM_DESIGN.md`](./SYSTEM_DESIGN.md) — Mermaid data-flow diagram, module breakdown, API integration strategy, security notes |
